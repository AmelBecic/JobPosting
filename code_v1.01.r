...
# Lets create test data that will be 20%
testData <- createDataPartition(y = spData_df$fraudulent, times = 1, p = 0.2, list= FALSE)
# Create train data that will be used for training our algorithm 80% of data portion
trainData <- spData_df[-testData, ]
# Test data 20% portion 
...
## Alternative solution : using Support Vector Machines (SVM)
...
svmAlgorithm <- train(fraudulent ~ .,data = trainData, 
                      ...
                      tuneGrid = grid_svm, trControl = trainControl)


## Evaluating alternative model performance ---
...
# Let's see will model for knn (since it is better than svm for this case) if we increase k to  tune up to 10
